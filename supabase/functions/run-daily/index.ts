...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const fitThreshold = 80; // Only draft for jobs with score >= 80
      ...
      try {
        ...
        try {
          ...
          try {
            ...
            fitScore = retryParsed.score || retryParsed.fit_score || 70;
            reasons = retryParsed.reasons || retryParsed;
          } catch {
            console.log("Using default fit score after retry");
          }
        }
    ...
    for (const { match, job } of scoredMatches) {
      try {
        ...
        try {
          ...
          // If not JSON, treat the whole response as cover letter
